...
export function SourcesStrip({ tweaks, stats }) {
  const loading = !stats || stats.loading;
  const sources = stats?.sources ?? [];
  const total = stats?.total ?? 0;
  const hasData = !loading && sources.length > 0;
    ...
    <section id="sources" style={{ padding: '80px 0', borderTop: '1px solid var(--line)', background: 'var(--bg-2)' }}>
      <div className="container">
        ...
        <h2 className="reveal" style={{ margin: 0, fontSize: 'clamp(24px, 2.8vw, 34px)', letterSpacing: '-0.02em', fontWeight: 600, marginBottom: 40 }}>
          {hasData ? (
            <>Live from <span style={{ color: tweaks.accent }}>{total.toLocaleString()}</span> events across {sources.length} {sources.length === 1 ? 'platform' : 'platforms'}.</>
          ) : loading ? (
            <>Counting live events across platforms…</>
          ) : (
            <>No live events right now — the next sync will refresh this.</>
          )}
        </h2>
              ...
              <div key={s.name} className="reveal" style={{
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div className="mono" style={{ fontSize: 12, color: 'var(--fg-3)', marginTop: 4 }}>
                    {s.count.toLocaleString()} live {s.count === 1 ? 'event' : 'events'}
                  </div>
                </div>
